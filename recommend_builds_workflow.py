...
def get_args():
    ...
    parser.add_argument('--pipeline_fk', required=False, action='store', default=None,
                        help='pipeline id')
...
class Uts:
    ...
    def update_pipeline(self, pipeline_id, update_data):
        final_products = []
        ...
        result = requests.post(self.server_url + '/v1/api/testdata/update_pipeline', json=data)
        check_http(result)
        return result.text
...
def main(args):
    ...
    if args.pipeline_fk:
        pipeline_id = uts_obj.finish_pipeline(pipeline_id=pipeline_fk, finish_data=pipeline_details)
    else:
        pipeline_id = uts_obj.update_pipeline(pipeline_id=pipeline_fk, update_data=pipeline_details)
    print(pipeline_id)
    return pipeline_id
...
# Start pipeline with sb build
pipeline_fk=$(python3 recommend_builds.py --pipeline_name=bundle_test --build_id=41019398 --build_system=sb --product=sddc-bundle --pipeline_build_number=2222)

Save pipeline_fk

pipeline_fk=$(python3 recommend_builds.py --pipeline_name=bundle_test --build_id=17102532 --build_system=ob --product=sddc-bundle --pipeline_build_number=2222 --pipeline_fk=5fa3e732b4abaa60a824a1f5)
