...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    replace:{
      log:{
        src: ['../ozone-scripts/ehc-python-scripts/conf/log.conf'],
        overwrite: true,
        replacements:[{
          from: 'C:\\fehc.log',
          to: '/data/log/ozone/fehc_python.log'
        }]
      },
      config:{
        src: ['../ozone-scripts/ehc-python-scripts/common/__init__.py'],
        overwrite: true,
        replacements:[{
          from: /\\\.\.\\conf\\log\.conf/,
          to: '/../conf/log.conf'
      ...
      constants:{
        src: ['../ozone-scripts/ehc-python-scripts/conf/constants.py'],
        overwrite: true,
        replacements:[{
          from: 'conf\\\\scriptConfig.json',
          to: 'conf/scriptConfig.json'
        },{
          from: /(orch_vm[^]+?)(C:\\\\ehc)([^]+execution_vm[^]+)/,
          to: '$1/tmp$3'
        }]
      },
      basedir:{
        src: ['../ozone-scripts/ehc-python-scripts/**/*.py'],
        overwrite: true,
        replacements:[{
          from: /''\.join\(\[BASEDIR, "\\\\"/g,
          to: '\'\'\.join\(\[BASEDIR, "/"'
          ...
          from: /"\{0\}\\\\\{1\}"\.format\(BASEDIR/g,
          to: '"\{0\}/{1\}"\.format\(BASEDIR'
        },{
          from: /BASEDIR,\s*'\\\\'/g,
          to: 'BASEDIR, \'/\''
        },{
          from: /"\{0\}\\\\\{1\}"\.format\(constants\.REMOTE_EXECUTION_PARAMS\['orch_vm']\['tmpdir']/g,
          to: '"\{0\}/{1\}"\.format\(constants\.REMOTE_EXECUTION_PARAMS\[\'orch_vm\']\[\'tmpdir\']'
        },{
          from: /''\.join\(\[constant\.REMOTE_EXECUTION_PARAMS\['orch_vm'\]\['tmpdir'\], '\\\\'/g,
          to: '\'\'\.join\(\[constant\.REMOTE_EXECUTION_PARAMS\[\'orch_vm\'\]\[\'tmpdir\'\], \'/\''
        }]

      },
      pywbem_ssl_fix:{
        src: ['../ozone-scripts/ehc-python-scripts/common/pywbemClient.py'],
        overwrite: true,
        replacements:[{
          from: /pywbem\.WBEMConnection\(applianceUrl, credTuple\)/g,
          to: 'pywbem.WBEMConnection(applianceUrl, credTuple, no_verification=True)'
        }]
      }
    },
    jsdoc : {
    ...
    usebanner: {
      python: {
        options: {
          position: 'top',
          banner: grunt.file.read('copyright.py'),
          linebreak: true,
          replace: true
        },
        files: {
          src: [  /*'../ozone-scripts/ehc-python-modules/!**!/!*.py',
                  '../ozone-scripts/ehc-ansible-project-templates/!**!/!*.py',*/
                  //'../ozone-scripts/ehc-ansible-project-templates/EHC41/_test_workflow.yml'
                  '../ozone-scripts/ehc-ansible-project-templates/**/*.yml',
          ]
        }
      },
      javascript: {
        options: {
          position: 'top',
          banner: grunt.file.read('copyright.js'),
          linebreak: true,
          replace: true
        },
        files: {
    ...
    filerev: {
      dist: {
        src: [
          ...
          '<%= yeoman.dist %>/<%= yeoman.client %>/assets/images/!(product_icons){,*/}*.{png,jpg,jpeg,gif,webp,svg}'
        ]
      }
    },
  ...
  grunt.registerTask('apply-banner', function (target) {
    grunt.task.run([
      'usebanner'
    ]);
  });

  grunt.registerTask('linuxify-python', function (target) {
    grunt.task.run([
      'replace'
    ]);
  });
  ...
  grunt.registerTask('serve', function (target) {
    ...
    //A custom prod build on serve. Used temporarily untill issue with grunt serve:dist is resolved
    if (target === 'prod') {
      return grunt.task.run([
        'clean:server',
        'env:all',
