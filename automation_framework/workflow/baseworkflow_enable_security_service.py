...
from auc.validate_enable_security.enable_and_validate_security_tas_api import ExecuteTasEnableSecurityAPI
from auc.validate_enable_security.enable_and_validate_security_mw_api import ExecuteMwEnableSecurityAPI
...
class BaseWorkflowSecurityService(object):
    """
    ...
    def execute_tas_enable_security_api(self, linux_policy_id, win_policy_id,
                                        vm_hostname, vm_ip, vm_id, vm_rid, task_id):
        """
        ...
        self.ctx_in = {'tas_ip': self.configyaml.get_config('TAS_DETAILS',
                       ...
                       'vm_ip': vm_ip,
                       'linux_policy_id': linux_policy_id,
                       'win_policy_id': win_policy_id,
                       'vm_id': vm_id,
                       'vm_rid': vm_rid,
                       'task_id': task_id,
        ...
        result = ExecuteTasEnableSecurityAPI(
            self.execute_tas_enable_security_api.__name__,
            ctx_in=self.ctx_in, ctx_out="").run()
        return result

    def execute_mw_enable_security_api(self, linux_policy_id, win_policy_id,
        ...
        result = ExecuteMwEnableSecurityAPI(
            self.execute_mw_enable_security_api.__name__,
            ctx_in=self.ctx_in, ctx_out="").run()
        return result
