...
class ExecuteMwEnableSecurityAPI(BaseUseCase):
    def test_execute_mw_enable_security_api(self):
        """
        ...
        try:
            enable_security_resp = req.post(MIDDLEWARE_API_ENABLE_SECURITY.
                                            format(self.mw_ip),
        ...
        if enable_security_resp.status_code == 200:
            ...
            time.sleep(120)
            command = 'tail -n 30 /var/log/middleware/worker.log'
            ...
            if matchObj:
                message = 'Unable to enable security from middleware.'
                logger.error(message)
                logger.debug(message)
                logger.info(result['output'])
                security_result = "FAIL"
            else:
                logger.info('Enable security API is Successfully executed')
                logger.info(result['output'])
        else:
            security_result = "FAIL"
    ...
    def run_test(self):
        mw_result = self.test_execute_mw_enable_security_api()
        logger.info("Middleware Enable SECURITY API RESULT = %s" % mw_result)
        return mw_result

    def _finalize_context(self):
        assert self.ctx_out == "PASS", "Could not initiate enable security API from Middleware"
