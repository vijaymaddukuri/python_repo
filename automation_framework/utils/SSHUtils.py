...
class SSHUtil(object):
    ...
    def __init__(self, host, username, password, keyfilepath=None,
                 keyfiletype=None, port=22, timeout=50):
        """
        ...
        try:
            # Paramiko.SSHClient can be used to make connections to the
            # remote server and transfer files
            ...
            # Setting the missing host key policy to AutoAddPolicy will silently add
            # any missing host keys. Using WarningPolicy, a warning message will be
            # logged if the host key is not previously known but all host keys will
            # still be accepted. Finally, RejectPolicy will reject all hosts which key
    ...
    def execute_command(self, command, get_pty=True, allowed_return_codes=[0, 1],
        ...
        try:
            if command is not None:
                ...
                stdin, stdout, stderr = self.client.exec_command(command, timeout=50,
                                                                 get_pty=get_pty)
        ...
        try:
            ...
            if ftp_client is not None:
                ...
                if not multiple_files:
                    if files:
                        remote_file_path = os.path.join(remote_file_path, files)
                        local_path = os.path.join(local_path, files)
                    ftp_client.get(remote_file_path, local_path)
                    print("Successfully imported files from remote machine to local machine "
                          "- {} >>> {}".format(remote_file_path, local_path))
                    return_data['status'] = True
                else:
                    # Upload file to the remote machine
                    for file in files:
