...
logfile = datetime.datetime.now().strftime('mw_console_%H_%M_%d_%m_%Y.log')
...
class Middleware:
    ...
    def __init__(self, config_file_path=yamlpath):
        ...
        self.tenant_id = get_config('MW_DETAILS', 'TENANT_ID', self.yaml_file_path)
        ...
        self.mw_header = MIDDLEWARE_SERVICE_HEADER
    ...
    def upload_yaml_files(self):
        ...
        local_mw_yaml_path = '{}{}'.format(present_dir, '/config/middleware/config.yaml')
        local_worker_yaml_path = '{}{}'.format(present_dir, '/config/worker/config.yaml')
    ...
    def check_services(self):
        ...
        command = "sudo systemctl status mws.service"
        result = self.ssh_obj.execute_command(command)
        ...
        command = "sudo systemctl status bws.service"
        result = self.ssh_obj.execute_command(command)
    ...
    def execute_middleware_health_check(self):
        ...
        try:
            health_check_resp = req.get(MIDDLEWARE_HEALTH_CHECK.format(self.mw_ip),
    ...
    def execute_middleware_enable_backup(self):
        ...
        try:
            enable_backup_resp = req.post(MIDDLEWARE_ENABLE_BACKUP.format(self.mw_ip),
    ...
    def execute_middleware_enable_security(self):
        ...
        try:
            enable_security_resp = req.post(MIDDLEWARE_ENABLE_SECURITY.format(self.mw_ip),
        ...
        if enable_security_resp.status_code == 200:
            ...
            if "No route to host" in return_value:
    ...
    def execute_middleware_enable_monitoring(self):
        ...
        try:
            enable_monitoring_resp = req.post(MIDDLEWARE_ENABLE_MONITORING
                                              .format(self.mw_ip),
    ...
    def execute_middleware_enable_vulnerability(self):
        ...
        enable_vulnerability_data = {'TenantID': self.tenant_id,
                                     'VirtualMachineIPAddress': self.vm_ip}
        try:
            enable_vulnerability_resp = req.post(MIDDLEWARE_ENABLE_VULNERABILITY
                                                 .format(self.mw_ip),
    ...
    def get_service_ip_from_consul(self):
        ...
        for line in result['output'].readlines():
            return_value = return_value + line
        resp_json = json.loads(return_value)
    ...
    def validate_mw_bb_server_connection(self, server_ip, server_port):
        ...
        if "HTML for static distribution" in return_value:
    ...
    def validate_vulnerability_endpoint(self):
        ...
        auth = b64encode(convertToBytes).decode("ascii")
        header = VULNERABILITY_SERVICE_HEADER
