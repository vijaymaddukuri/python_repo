...
class RestLib(Session):
    ...
    def get_policy_id_by_name(self, hostname, name):
        """
        Description: Get VirtuStream Policy ID by Name
        Args:
        :param hostname: Hostname of the server
        ...
        """
        try:
            logger.info('Begin the testcase to get VirtuStream Policy ID')
            url = 'http://{}/{}'.format(hostname, VIRTUSTREAM_POLICIES)
            logger.info('URL is {}'.format(url))
            response = self.get(url)
            logger.info("The status of REST API command is {}".format(response.status_code))
            if not response.ok:
                logger.error('Failed to get the VirtuStream Policy data with the above REST API')
                return False
            policy_list = response.json()
            for policy in policy_list:
                if policy['name'] == name:
                    logger.info("For the VirtuStream Policy  - {}, the VirtuStream ID is {}"
                                .format(policy['name'], policy['id']))
                    return policy['id']
            logger.error("From the VirtuStream Policy list, unable to find the VirtuStream policy {}".format(name))
            return False
        except Exception as err:
            logger.error('Exception while get VirtuStream policy id by name')
            logger.error(err)
            return False
